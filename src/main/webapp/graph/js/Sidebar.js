...
/**
 * Specifies the URL of the echarts image.
 */
Sidebar.prototype.lineChart = STENCIL_PATH + '/clipart/Graph_128x128.png';
 ...
 * Adds the general palette to the sidebar.
 */
Sidebar.prototype.addEchartPalette = function(expand)
{
    var sb = this;

    var fns = [
        //sidebar添加echarts组件
        this.addEntry('lineChart', mxUtils.bind(this, function()
        {
            var arrs=[];
            for (var i = 0; i < 7; i++) {
                var s=Math.ceil(Math.random()*500);
                arrs.push(s);
            }
            var option = {
                xAxis: {
            ...
            var echarthtml="<div id='echartS' style='width: 500px; height: 260px;background: url("+sb.lineChart+") no-repeat;background-size: contain;'></div>";
            var cells = [new mxCell(echarthtml, new mxGeometry(0, 0, 300, 240), 'html=1;strokeColor=none;fillColor=none;overflow=fill;')];
            cells[0].vertex = true;
            cells[0].localData=JSON.stringify(option);
            cells[0].type = "Echarts";

            return this.createVertexTemplateFromCells(cells, 300, 240, 'lineChart');
        })),
        this.addEntry('lineChart2', mxUtils.bind(this, function()
        {
            var arrs=[];
            for (var i = 0; i < 7; i++) {
                var s=Math.ceil(Math.random()*500);
    ...
    this.addPaletteFunctions('echarts', mxResources.get('echarts'), (expand != null) ? expand : true, fns);
 ...
 * Creates a drag source for the given element.
 */
Sidebar.prototype.cloneCell = function(elt, dropHandler, preview, cells, bounds)
{
...
Sidebar.prototype.addClickHandler = function(elt, ds, cells)
{
    ...
    ds.mouseUp = function(evt)
    {
        ...
        var option = JSON.parse(cells[0].localData);
        // if (cells[0].type == "Echarts"){
        //     // 初始化
        //     var myChart =echarts.init($(cells[0].value)[0]);
        //     myChart.setOption(option);
        // }
        var ECElement=document.querySelectorAll('#echartS');
        if( ECElement.length==1){
            return
        }else {
            var arrayOption=[];
            var arrayChart=[];
            for (let i = ECElement.length-1 ; i < ECElement.length; i++) {
                ECElement[i].style.background = 'none';
                // 初始化
                ...
                function resetIput(dataSS,myCharts,options){

                    var num2=$('tr')[1].children[0].children[0].value;

                    // 上方input框
                    if ($('tr')[0].children[1]!==undefined) {
                        for (let i = 0; i < num; i++) {
                            $('tr')[0].removeChild($('tr')[0].children[1])
                        }
                        // 下方input框
                        if ($('tr')[1].children[1]!==undefined) {
                            for (let i = 0; i < num; i++) {
                                $('tr')[1].removeChild($('tr')[1].children[1])
                            }
                        }
                        // 判断用是否输入值
                        if (num2!=='') {
                            num=num2;
                            $('tr')[1].children[0].children[0].name=num2;
                        }
                    }
                    createInput(num,dataSS,myCharts,options)
                }

                function createInput(num,dataSS,myCharts,options){
                    // 添加
                    for (let i = 0; i < num; i++) {
                    ...
                    for (let i = 0; i < num; i++) {
                        ...
                        if (dataSS[i]==undefined){
                            inputs.value='0';
                        }else {
                            inputs.value=dataSS[i]
                        }
                        var tds=document.createElement('td');
                        inputs.style="width: 40px";
                        tds.style="width: 40px";
                        tds.appendChild(inputs)
                        $('tr')[1].append(tds)
                        dataArr.push($('tr')[1].children[i+1].children[0].value)
                ...
                ECElement[i].onclick=function (e) {
                    // 查找点击元素索引
                    var ECElements=Array.from(document.querySelectorAll('#echartS'));
                    var item = e.target;
                    var itemParent=item.parentNode.parentNode;
                    var cellIndex=ECElements.indexOf(itemParent)
                    // echarts取数据
                    var myCharts=arrayChart[0];
                    ...
                    if(options.series[0].areaStyle==null){
                        $('#contentConfig input')[0].checked=false
                    }else {
...
Sidebar.prototype.addImagePalette = function(id, title, prefix, postfix, items, titles, tags)
{
    ...
    for (var i = 0; i < items.length; i++)
    {
        (mxUtils.bind(this, function(item, title, tmpTags)
        {
            ...
            fns.push(this.createVertexTemplateEntry('shape=image;html=1;labelBackgroundColor=#ffffff;image=' + prefix + item + postfix,

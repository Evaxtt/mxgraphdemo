...
EditorUi.prototype.showHtmlDialog = function(btnLabel, fn)
{
    ...
    var CustomDialog = function(editorUi, content, okFn, cancelFn, okButtonText, buttonsContent, hideCancel)
    {
        var div = document.createElement('div');
 ...
 *
 */
EditorUi.prototype.createHtml = function(publicUrl, fn)
{
    ...
    {
        data.xml = mxUtils.getXml(this.editor.getGraphXml());
    }

    var value = '<div class="mxgraph" style="max-width:100%;border:1px solid transparent;" data-mxgraph="' + mxUtils.htmlEntities(JSON.stringify(data)) + '"></div>';

    var s2 =  this.drawHost + '/graph/lib/viewer.min.js';
    var scr = '<script type="text/javascript" src="' + s2 + '"></script>';
    var basename = "preview";
    ...
    //预览
    sessionStorage.removeItem("graphXml");
	sessionStorage.setItem("graphXml",data.xml);
    window.open('graph/preview.html', 'newwindow', 'height=100%, width=100%, top=0,' +
            ' left=0, toolbar=no, menubar=no, scrollbars=no, resizable=yes, location=no, status=no')
 ...
 *
 */
EditorUi.prototype.createXml = function(fn)
{
    var data = {};
    data.xml = mxUtils.getXml(this.editor.getGraphXml());
    fn(data.xml);

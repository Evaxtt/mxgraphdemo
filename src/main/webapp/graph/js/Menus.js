...
Menus.prototype.init = function()
{
	...
	{
		this.addMenuItems(menu, ['new', 'open', '-', 'save', 'saveAs', '-', 'import', 'export', '-', 'pageSetup', 'print','exportHtml','publish'], parent);
	})));
    ...
    editorUi.actions.put('exportHtml', new Action( 'export HTML...', function()
    {
		editorUi.showHtmlDialog(mxResources.get('export'), function(publicUrl,editLink)
		{
			editorUi.createHtml(publicUrl, editLink, mxUtils.bind(this, function(html, scriptTag)
				{
    ...
    editorUi.actions.put('publish', new Action(mxResources.get('preview_page'), function()
    {
        editorUi.createXml(mxUtils.bind(this, function(xml)
        {
            window.sessionStorage.setItem( 'publishXml', xml);
            window.open(window.view_FORM)
        }));
    }));
};
